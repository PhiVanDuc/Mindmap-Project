...
export default function MindmapTable() {
    ...
    const handleClickDeleteMindmap = (id) => {
        ...
        const row = thisButton.closest(".table-row");
        
        dispatch(fetchDeleteMindmapList(id));

        if (status === "rejected") {
            toast.error('Xóa mindmap thất bại!', {
                position: "top-right",
                autoClose: 3000,
                    ...
                    if (session?.user?.email === userEmail) {
                        return (
                            <div className="table-row" key={ id }>
                                ...
                                <div className="table-data">
                                    <h4>
                                        <Link href={`/my-mindmap/${id}`}>{ name }</Link>
                                    </h4>
                                    <p>{ description }</p>
                                </div>
                                ...
                                <div className="table-data">
                                    ...
                                    <span className='icon-delete' data-id={id} onClick={() => {handleClickDeleteMindmap(id)}}>
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
